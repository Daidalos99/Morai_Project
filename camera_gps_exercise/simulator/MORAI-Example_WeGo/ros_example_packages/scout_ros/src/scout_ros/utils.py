...
def findLocalPath(ref_path,status_msg):
    ...
    if current_waypoint+50 > len(ref_path.poses) :  # 50은 local path planner에서 앞에 얼만큼의 인덱스(거리)까지 볼 것인지를 정함
...
def latticePlanner(ref_path,global_vaild_object,vehicle_status,current_lane):
    ...
    look_distance=int(vehicle_status[3]*3.6*0.2*2)  # vehicle_status[3]: 속도값(velocity_x)
    if look_distance < 3 :
